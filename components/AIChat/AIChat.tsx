...
import { ArrowLeft, Save, MessageSquare, X, Quote, Paperclip, Terminal, Search, Edit, PenTool, BookOpen, FileText, File } from "lucide-react";
...
import { Message as MessageType, Change, InputMode } from "@/types/chatTypes";
...
const AIChat: React.FC<AIChatProps> = ({ editorContent, updateEditorContent }) => {
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSelectionButtonVisible, setIsSelectionButtonVisible] = useState(false);
  ...
  const handleSendMessage = useCallback(async () => {
    if (!input.trim() && selectedDocuments.length === 0 && selectedSnippets.length === 0) return;

    setIsSubmitting(true);
    setInput("");
    setSelectedSnippets([]);

    const snippetsText = selectedSnippets.map((snippet) => `"${snippet}"`).join(" ");
  ...
  }, [input, selectedDocuments, selectedSnippets, messages, inputMode, editorContent, selectedSource, scrollToBottom]);
